...
void heapifyDown(int* arr, int i, int size) {
    while (i * 2 + 1 < size) { // heapify down until complete
        int lIdx = i * 2 + 1;
        int rIdx = i * 2 + 2;
        
        int smallIdx = i; // index of the smallest value of the 3
        if (arr[smallIdx] > arr[lIdx]) { // is left child smaller?
            smallIdx = lIdx;
        }

        if (rIdx < size && arr[smallIdx] > arr[rIdx]) { // right child smaller?
            smallIdx = rIdx;
        }

        if (smallIdx == i) { // parent is already smallest, stop heapifying
            break;
        }
